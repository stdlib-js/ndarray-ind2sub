...
function ind2sub( shape, idx, options, out ) {
	...
	if ( arguments.length === 4 ) {
		...
		if ( typeof out !== 'object' || out === null ) {
			throw new TypeError( format( '0jx5j,LP', out ) );
		}
		dest = out;
	} else {
		dest = options;
		if ( typeof dest !== 'object' || dest === null ) {
			throw new TypeError( format( '0jx5j,LP', dest ) );
		}
	}
	if ( !isNonNegativeIntegerArray( shape ) ) {
		throw new TypeError( format( '0jx5k,LN', shape ) );
	}
	if ( !isInteger( idx ) ) {
		throw new TypeError( format( '0jx5l,LO', idx ) );
	}
